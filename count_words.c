...
{
  /* this part of the code isnt correct because when i say char *words = "words" i save it as readonly data.
   * so when i try to change words[strlen(words)-1] = '\0' im trying to change a read only space which is a
   * segmentation fault
   *char *words = "words";
  if (count == 1)
